...
<body style="font-family:Arial,sans-serif;line-height:1.6;color:#1f2937;max-width:600px;margin:0 auto;padding:24px;">
  <div style="border-bottom:3px solid #2563eb;padding-bottom:16px;margin-bottom:24px;">
    <h1 style="margin:0;font-size:20px;color:#111827;">Teamwork Homecare</h1>
  </div>
  ${body}
  <p style="margin-top:32px;font-size:12px;color:#6b7280;">This is an automated message from Teamwork Homecare.</p>
</body>
</html>`;
}
...
export class HiringEmailService {
  ...
  static async sendInterviewFailed(input: { name: string; email: string }): Promise<boolean> {
    const body = `
      ...
      <p>We appreciate your interest in Teamwork Homecare and wish you the best.</p>
    `;
    return EmailService.send({
      to: { email: input.email, name: input.name },
  ...
  static async sendRecruited(input: { name: string; email: string }): Promise<boolean> {
    const body = `
      <p>Dear ${input.name},</p>
      <p>Welcome to Teamwork Homecare — your recruitment is complete.</p>
